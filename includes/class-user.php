...
class User {
    ...
    public function __construct() {
        ...
        if(isset($_COOKIE[$this->_cookie_user_id]) && isset($_COOKIE[$this->_cookie_user_token])) {
            ...
            if($get_user->num_rows > 0) {
                ...
                /* get devices */
                $this->_data['devices'] = $this->getDevices(  $this->_data['user_id'], true );
                /* can write articles */
	...
	/**
     * get_friends2 -- adds Email.
     * 
     * @param integer $user_id
     * @param integer $offset
     * @return array
     */
    public function get_friends2($user_id, $offset = 0) {
        global $db, $system;
        $friends = array();
        $offset *= $system['min_results_even'];
        /* get the target user's privacy */
        ...
        $get_friends = $db->query(sprintf('SELECT users.user_id, users.user_email, users.user_name, users.user_firstname, users.user_lastname, users.user_gender, users.user_picture FROM friends INNER JOIN users ON (friends.user_one_id = users.user_id AND friends.user_one_id != %1$s) OR (friends.user_two_id = users.user_id AND friends.user_two_id != %1$s) WHERE status = 1 AND (user_one_id = %1$s OR user_two_id = %1$s) LIMIT %2$s, %3$s', secure($user_id, 'int'), secure($offset, 'int', false), secure($system['min_results_even'], 'int', false) )) or _error(SQL_ERROR_THROWEN);
        if($get_friends->num_rows > 0) {
            while($friend = $get_friends->fetch_assoc()) {
                $friend['user_picture'] = get_picture($friend['user_picture'], $friend['user_gender']);
				unset( $friend['user_gender']);
                /* get the connection between the viewer & the target */
                $friend['initial'] = substr($friend['user_firstname'],0,1).substr($friend['user_lastname'],0,1);
			   // $friend['connection'] = $this->connection($friend['user_id']);
                $friends[] = $friend;
            }
        }
    ...
    public function get_followings($user_id, $offset = 0) {
        ...
        $followings = array();
        $offset *= $system['min_results_even'];
        $get_followings = $db->query(sprintf('SELECT users.user_id, users.user_name, users.user_firstname, users.user_lastname, users.user_gender, users.user_picture, users.user_subscribed, users.user_verified FROM followings INNER JOIN users ON (followings.following_id = users.user_id) WHERE followings.user_id = %s LIMIT %s, %s', secure($user_id, 'int'), secure($offset, 'int', false), secure($system['min_results_even'], 'int', false) )) or _error(SQL_ERROR_THROWEN);
        if($get_followings->num_rows > 0) {
            while($following = $get_followings->fetch_assoc()) {
    ...
    public function get_followers($user_id, $offset = 0) {
        ...
        $followers = array();
        $offset *= $system['min_results_even'];
        $get_followers = $db->query(sprintf('SELECT users.user_id, users.user_name, users.user_firstname, users.user_lastname, users.user_gender, users.user_picture, users.user_subscribed, users.user_verified FROM followings INNER JOIN users ON (followings.user_id = users.user_id) WHERE followings.following_id = %s LIMIT %s, %s', secure($user_id, 'int'), secure($offset, 'int', false), secure($system['min_results_even'], 'int', false) )) or _error(SQL_ERROR_THROWEN);
        if($get_followers->num_rows > 0) {
            while($follower = $get_followers->fetch_assoc()) {
        ...
        if($get_notifications->num_rows > 0) {
            while($notification = $get_notifications->fetch_assoc()) {
                ...
                switch ($notification['action']) {
                    ...
                    case 'before_session_start':
                        $notification['icon'] = "fa-calendar";
                        $notification['url'] = $system['system_url'].'/events/'.$notification['node_url'];
                        $notification['message'] = " '".html_entity_decode($notification['node_type'], ENT_QUOTES)."' " .__("session is going to start.");
                        break;

                    case 'after_session_start':
                        $notification['icon'] = "fa-calendar";
                        $notification['url'] = $system['system_url'].'/events/'.$notification['node_url'];
                        $notification['message'] = " '".html_entity_decode($notification['node_type'], ENT_QUOTES)."' " .__("session has been started.");
                        break;
    ...
    public function edit_page($page_id, $edit, $args) {
        ...
        switch ($edit) {
            case 'settings':
                ...
                if(!isset($args['page_verified'])) {
                    $args['page_verified'] = $page['page_verified'];
                }
                if(!isset($args['page_description'])) {
    ...
    public function create_event($args = []) {
        ...
        /* validate title */
        if(is_empty($args['invite_session'])) {
            throw new Exception(__("You must enter a name for your session"));
        }
        if(strlen($args['invite_session']) < 3) {
            throw new Exception(__("Session name must be at least 3 characters long. Please try another"));
        }
        /* validate start & end dates */

        if (!$args['invite_open']) {
           if(is_empty($args['invite_start_date_time'])) {
            throw new Exception(__("You have to enter the event start date"));
            }
            if(is_empty($args['invite_end_date_time'])) {
                throw new Exception(__("You have to enter the event end date"));
            }
            if(strtotime($args['invite_start_date_time']) > strtotime($args['invite_end_date_time'])) {
                throw new Exception(__("Event end date must be after the start date"));
            }
        }else{
            $args['invite_start_date_time'] = -1;
            $args['invite_end_date_time'] = -1;
        }
        
        // for now we are doing privacy is public
        $args['privacy'] = 'public';
        /* validate privacy */
        if(!in_array($args['privacy'], array('secret','closed','public'))) {
       ...
       /* $check = $db->query(sprintf("SELECT * FROM events_categories WHERE category_id = %s", secure($args['category'], 'int') )) or _error(SQL_ERROR_THROWEN);
        ...
        if(is_empty($args['connection_type'])) {
            throw new Exception(__("You have to enter the type"));
        }
        if (isset($args['invite_friends']) && is_array($args['invite_friends'])) {
            $invited_friends = $args['invite_friends'];
        }

        if(count($invited_friends) < 1) {
            throw new Exception(__("You have to invite friends"));
        }
        if(is_empty((int) $args['invite_session_type'])) {
            throw new Exception(__("You have to select session type"));
        }
        // taking variable is null so that we dont need to make changes in query string

        $args['location'] = null;
        $args['description'] = null;

        $db->query(sprintf("INSERT INTO events (session_type_id, connection_type, always_on, event_privacy, event_admin, event_category, event_title, event_location, event_description, event_start_date, event_end_date, event_date) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)", secure($args['invite_session_type']), secure($args['connection_type']), secure($args['invite_open'] ? 1 : 0), secure($args['privacy']), secure($this->_data['user_id'], 'int'), secure($args['category'], 'int'), secure($args['invite_session']), secure($args['location']), secure($args['description']), secure(date('Y-m-d H:i', strtotime($args['invite_start_date_time']))), secure(date('Y-m-d H:i', strtotime($args['invite_end_date_time']))), secure($date) )) or _error(SQL_ERROR_THROWEN);
        /* get event_id */
        $event_id = $db->insert_id;

        $notification = array();
        foreach ($invited_friends as $user_id) {
            $notification = array('to_user_id' => $user_id, 'from_user_id' => $this->_data['user_id'], 'action' => 'session_invitation', 'node_type' => $args['invite_session'], 'node_url' => $event_id);

            $this->post_notification($notification);
            
            $db->query(sprintf("INSERT INTO events_members (event_id, user_id, is_invited) VALUES (%s, %s, %s)", secure($event_id), secure($user_id), secure(1) )) or _error(SQL_ERROR_THROWEN);
        }
        /* insert custom fields values */
        // if($custom_fields) {
        //     foreach($custom_fields as $field_id => $value) {
    ...
    public function sign_in($username_email, $password, $remember = false) {
		...
		//Passes this sha token back for mqtt.
		$sha = sha1( $user['user_password']);
		
		//Passed the username and user cover back to post.
		$uinfo = [];
		$uinfo['user_id']= $user['user_id'];
		$uinfo['user_name']= $user['user_name'];
		$uinfo['user_picture']= get_picture($user['user_picture'], $user['user_gender']);//"https://qrs-connect.com/content/uploads/".$user['user_picture'];
     ...
     *   $this->_logged_in
     */
    public function getPostId($source, $return_info = false) {
        global $db;
       //Not the best solution -- Source look up.  THe mid and json are combined.
	    $query = $db->query(sprintf("SELECT post_id, audio_id FROM `posts_audios`  WHERE SOURCE LIKE '%s';", "%".$source."%" )) or _error(SQL_ERROR_THROWEN);
        if( !file_exists("/var/www/html/content/uploads/$source") ){
		...
		if($query->num_rows > 0) {
            if($return_info) {
				$data = [];
				 ...
				 $data['fileContent'] = file_get_contents("qrs-connect.com/posts/".$data['post_id'] );
                return $data;
            }
            return true;
        }
     ...
     * 
     */
    public function getUserCalendar($user_id, $return_info = false) {
        global $db;
        $query = $db->query(sprintf("SELECT MUSIC_ID ,TITLE,RECORDED_DATE,ACTIVITY_CATEGORY1, ACTIVITY_CATEGORY2,DEVICE FROM MUSIC_LIBRARY WHERE userid = %s", $user_id )) or _error(SQL_ERROR_THROWEN);
       // $query = $db->query("SELECT DISTINCT MUSIC_ID ,TITLE,RECORDED_DATE,ACTIVITY_CATEGORY1, ACTIVITY_CATEGORY2,DEVICE FROM MUSIC_LIBRARY WHERE userid = 2 ORDER BY RECORDED_DATE") or _error(SQL_ERROR_THROWEN);
        
		if($query->num_rows > 0) {
            if($return_info) {
				$data = [];
				while($row= $query->fetch_assoc()) {
               // $info = $row = $query->fetch_	assoc();
					$data[]=$row;
				}
                return $data;
            }
     ...
     * 
     */
    public function getUserCalendars($user_id, $calendar, $startDate, $endDate, $return_info = false) {
        global $db;
		//global $return;
		$calendar = $calendar - 1;
       // $query = $db->query(sprintf("SELECT MUSIC_ID ,TITLE,RECORDED_DATE,ACTIVITY_CATEGORY1, ACTIVITY_CATEGORY2,DEVICE FROM MUSIC_LIBRARY WHERE userid = %s", $user_id )) or _error(SQL_ERROR_THROWEN);
       //C $sql = sprintf("SELECT FILE_PATH , ACC_FILE_PATH, MUSIC_ID as music_id, RECORDED_DATE as startDate,RECORDED_DATE as endDate, TITLE AS title, ACTIVITY_CATEGORY1 as AC ,DEVICE AS description, ACTIVITY_CATEGORY2 AS calendarId FROM MUSIC_LIBRARY WHERE ACTIVITY_CATEGORY2 = %s AND (cast(RECORDED_DATE as date) BETWEEN cast('%s' as date) AND cast('%s' as date) ) AND userid = 2 ORDER BY RECORDED_DATE", $calendar, $startDate, $endDate);
		$sql = sprintf("SELECT FILE_PATH , ACC_FILE_PATH, MUSIC_ID as music_id, RECORDED_DATE as startDate,RECORDED_DATE as endDate, TITLE AS title, ACTIVITY_CATEGORY1 as AC ,DEVICE AS description, ACTIVITY_CATEGORY2 AS calendarId FROM MUSIC_LIBRARY WHERE ACTIVITY_CATEGORY2 = %s AND (cast(RECORDED_DATE as date) BETWEEN cast('%s' as date) AND cast('%s' as date) ) AND userid = %s ORDER BY RECORDED_DATE", $calendar, $startDate, $endDate,$user_id);
       // $sql = [$user_id,$startDate,$endDate];//"SELECT DISTINCT MUSIC_ID ,TITLE,UNIX_TIMESTAMP(RECORDED_DATE) as TS,RECORDED_DATE,ACTIVITY_CATEGORY1, ACTIVITY_CATEGORY2,DEVICE FROM MUSIC_LIBRARY WHERE ACTIVITY_CATEGORY2 = $calendar_type AND (cast(RECORDED_DATE as date) BETWEEN cast($startDate as date) AND cast($endDate as date) ) AND userid = 2 ORDER BY RECORDED_DATE;";
		//$query = $db->query(sprintf("SELECT DISTINCT MUSIC_ID ,TITLE,UNIX_TIMESTAMP(RECORDED_DATE) as TS,RECORDED_DATE,ACTIVITY_CATEGORY1, ACTIVITY_CATEGORY2,DEVICE FROM MUSIC_LIBRARY WHERE ACTIVITY_CATEGORY2 = %s AND (cast(RECORDED_DATE as date) BETWEEN cast(%s as date) AND cast(%s as date) ) AND userid = 2 ORDER BY RECORDED_DATE", $calendar_type, $startDate, $endDate) ) or _error(SQL_ERROR_THROWEN);
       // $return['sql']=$sql;
		$query = $db->query($sql);
		if($query->num_rows > 0) {
            if($return_info) {
     ...
     * 
     */
    public function getDevices($user_id, $return_info = false) {
        global $db;

        $query = $db->query(sprintf("SELECT * from devices WHERE userid = %s", $user_id )) or _error(SQL_ERROR_THROWEN);
		//$query = $db->query("SELECT * from devices WHERE userid = 2") or _error(SQL_ERROR_THROWEN);
        if($query->num_rows > 0) {
            if($return_info) {
				$data = [];
				while($row= $query->fetch_assoc()) {
     ...
     * 
     */
    public function getDevice($user_id, $return_info = false) {
        global $db;
		//Get gear.
		$query = $db->query(sprintf("SELECT * from gear WHERE userid = %s", $user_id )) or _error(SQL_ERROR_THROWEN);
		$gear = [];
        if($query->num_rows > 0) {
			while($row= $query->fetch_assoc()) {
				$gear[ $row['serial'] ]=$row;
        ...
        $query = $db->query(sprintf("SELECT serial,isOnline from devices WHERE isUserAllowed = 1 AND userid = %s", $user_id )) or _error(SQL_ERROR_THROWEN);
        if($query->num_rows > 0) {
            if($return_info) {
				$data = [];
				while($row= $query->fetch_assoc()) {
					//Copy in gear into this dataset.
					$row['gear']=$gear[$row['serial']];
					$data[]=$row;
				}
                return $data;
            }
     ...
     * 
     */
    public function setDevices($user_id, $device, $return_info = false) {
        global $db;
		
        //id, userid, serial, defaultGearId
		$query = $db->query(sprintf("Select * from devices WHERE userid = %s and serial = %s",secure($user_id), secure($device) )) or _error(SQL_ERROR_THROWEN);
        if($query->num_rows == 0) {
			$query = $db->query( sprintf("INSERT INTO devices ( `userid`, `serial`, `defauItGearId`, `isOnline`, `isUserAllowed`) VALUES ( %s, %s, 0,1, 1)",secure($user_id),secure($device) ) ) or _error(SQL_ERROR_THROWEN);	
		} else {
			/*while($row= $query->fetch_assoc()) {
				if($row['isUserAllowed']=="0"){*/			
			$query = $db->query( sprintf( "UPDATE devices set isUserAllowed = 1 where isUserAllowed = 0 and userid = %s AND serial = %s" ,secure($user_id),secure($device) ) ) or _error(SQL_ERROR_THROWEN);
		}
    }
     ...
     * 
     */
    public function setDeviceDisallow($user_id, $device, $return_info = false) {
        global $db;
        //id, userid, serial, defaultGearId
		$query = $db->query(sprintf("Select * from devices WHERE isUserAllowed = 1 and userid = %s and serial = %s",secure($user_id), secure($device) )) or _error(SQL_ERROR_THROWEN);
        if($query->num_rows >0) {
			$query = $db->query(sprintf("UPDATE devices set isUserAllowed = 0 where isUserAllowed = 0 and userid = %s AND serial = %s",secure($user_id),secure($device) ) ) or _error(SQL_ERROR_THROWEN);
		} 
    }
     ...
     * 
     */
    public function deleteDevices($user_id, $device) {
        global $db;
		//Check Existence then delete.
		$query = $db->query(sprintf("SELECT * from devices WHERE userid = %s and id = %s",secure($user_email),  secure($device) ) ) or _error(SQL_ERROR_THROWEN);
        if($query->num_rows > 0) {
			//Delete the id --> but check the user id matches the current user.  Not allowed to delete other users data.
			$query2 = $db->query(sprintf("Delete from devices where userid = %s and id = %s", secure($user_id), secure($device) )) or _error(SQL_ERROR_THROWEN);
		}
    }
     ...
     * 
     */
    public function setDeviceACL($user_email, $device, $return_info = false) {
        global $db;
        //id,allow(0/1),ipaddr,username,clientid,access,topic.
		//Do we also need to clear out all other device entries for that device?  
		// I believe no because the devices could be signed in by the same user.
		$topic = $device. "/#";
		$query = $db->query(sprintf("SELECT * from mqtt_acl WHERE username = %s and topic = %s",secure($user_email), secure($topic) ) ) or _error(SQL_ERROR_THROWEN);
        if($query->num_rows ==0) {
			$query = $db->query(sprintf("INSERT INTO `mqtt_acl`( `allow`, `ipaddr`, `username`, `clientid`, `access`, `topic`) VALUES (1,NULL,%s,NULL, 3, %s)",secure($user_email),secure($topic) ) ) or _error(SQL_ERROR_THROWEN);
		}
    }
     ...
     * 
     */
    public function deleteDeviceACL($user_email, $device, $return_info = false) {
        global $db;
        //id,allow(0/1),ipaddr,username,clientid,access,topic.
		//Do we also need to clear out all other device entries for that device?  
		// I believe no because the devices could be signed in by the same user.
		$topic = $device. "/#";
		$sql = sprintf("Delete from mqtt_acl WHERE username = %s and topic = %s",secure($user_email), secure($topic) );
		$query = $db->query($sql) or _error(SQL_ERROR_THROWEN);	
     ...
     * 
     */
    public function getGear($user_id, $return_info = false) {
          global $db;
		//Get gear.
		$query = $db->query(sprintf("SELECT * from gear WHERE userid = %s", $user_id )) or _error(SQL_ERROR_THROWEN);
		$gear = [];
	...
	/**
     * setDeviceOnline also passes to mosquitto userStatus
     * 
     * @param int $status --gives the 0/1 for offline/online
     * @param boolean $return_info
     * @return boolean|array
     * 
     */
    public function setDeviceOnline($device, $status, $return_info = false) {
        global $db;
		$sql = sprintf("UPDATE devices set isOnline = %s where serial = %s",$status,secure($device) );
		$query = $db->query( $sql ) or _error(SQL_ERROR_THROWEN);
		
		$query = $db->query(sprintf("SELECT userid from devices WHERE serial = %s", secure($device)  )) or _error(SQL_ERROR_THROWEN);
        if($query->num_rows > 0) {
            if($return_info) {
				$data = [];
				while( $row = $query->fetch_assoc() ) {
     ...
     * 
     */
    public function getMusicStats($user_id, $serial = false, $return_info = false) {
        global $db;
		if(!$serial){
			$query = $db->query(sprintf("SELECT COUNT(*) as songCount ,SUM(`notesOn`) as notesOn,SUM(`notesOff`) as notesOff, SUM(`pedalOn`) as pedalsOn, SUM(`pedalOff`) as pedalsOff, SUM(`songLength`) as totalTime FROM `MUSIC_LIBRARY` WHERE userid = %s GROUP BY `userid`", secure($user_id) )) or _error(SQL_ERROR_THROWEN);
		} else {
			$query = $db->query(sprintf("SELECT COUNT(*) as songCount ,SUM(`notesOn`) as notesOn,SUM(`notesOff`) as notesOff, SUM(`pedalOn`) as pedalsOn, SUM(`pedalOff`) as pedalsOff, SUM(`songLength`) as totalTime FROM `MUSIC_LIBRARY` WHERE userid = %s AND DEVICE = %s GROUP BY `DEVICE`, `userid`", secure($user_id), secure($serial) )) or _error(SQL_ERROR_THROWEN);
		}
        if($query->num_rows > 0) {
            if($return_info) {
				$data = [];
     ...
     * 
     */
    public function getMusicTypeCount($user_id, $serial = false, $return_info = false) {
        global $db;
		if($serial){
			 $query = $db->query(sprintf("SELECT substr(`file_path` FROM `MUSIC_LIBRARY` WHERE userid = %s AND serial = %s GROUP BY `userid`", secure($user_id) ,secure($serial) )) or _error(SQL_ERROR_THROWEN);
		} else {
			 $query = $db->query(sprintf("SELECT substr(`file_path` FROM `MUSIC_LIBRARY` WHERE userid = %s GROUP BY `userid`", secure($user_id) )) or _error(SQL_ERROR_THROWEN);
		}
    ...
    public function getAllUserFriendsList($user_id, $return_info = false) {

        global $db;

        $query1 = $db->query(sprintf('SELECT CONCAT(u.user_firstname, " ", u.user_lastname) as user_name, u.user_id FROM friends f
                                     INNER JOIN users u ON u.user_id = f.user_two_id
                                    WHERE f.user_one_id = %s', secure($user_id) )) or _error(SQL_ERROR_THROWEN);
    

        $query2 = $db->query(sprintf('SELECT CONCAT(u.user_firstname, " ", u.user_lastname) as user_name, u.user_id FROM friends f
                                      INNER JOIN users u ON u.user_id = f.user_one_id
                                      WHERE f.user_two_id = %s',  secure($user_id) )) or _error(SQL_ERROR_THROWEN);
    ...
    public function getAllSessionTypes() {
        ...
        $query = $db->query(sprintf('SELECT * FROM session_type' )) or _error(SQL_ERROR_THROWEN);
    ...
    public function checkUserHaveEvent(){
        global $db;
    ...
    static function pushNotificationBeforeSessionStart()
    {   

        global $db, $date;

        $get_events = $db->query(sprintf('SELECT event_id, event_admin, event_title FROM `events` WHERE TIME_TO_SEC(TIMEDIFF(event_start_date, CURRENT_TIMESTAMP)) BETWEEN 1 AND 300' )) or _error(SQL_ERROR_THROWEN);

        if($get_events->num_rows > 0) {
            while($row= $get_events->fetch_assoc()) {

                $get_members = $db->query(sprintf('SELECT user_id  FROM `events_members` WHERE `event_id` = %s',  secure($row['event_id']) )) or _error(SQL_ERROR_THROWEN);

                if($get_members->num_rows > 0) {
                    while($member_row= $get_members->fetch_assoc()) {

                        $check_notification = $db->query(sprintf('SELECT COUNT(*) As totalRecord FROM `notifications` WHERE `to_user_id` = %s AND `node_url` = %s AND node_type = %s',  secure($member_row['user_id']),  secure($row['event_id']),  secure('session_join') )) or _error(SQL_ERROR_THROWEN);

                        $check_notification_row = $check_notification->fetch_assoc();


                        if (empty($check_notification_row['totalRecord'])) {
                            $notification = array('to_user_id' => $member_row['user_id'], 'from_user_id' => $row['event_admin'], 'action' => 'before_session_start', 'node_type' => $row['event_title'], 'node_url' => $row['event_id']);

                            $action = 'before_session_start';

                            $db->query(sprintf("INSERT INTO notifications (to_user_id, from_user_id, action, node_type, node_url, time) VALUES (%s, %s, %s, %s, %s, %s)", secure($member_row['user_id'], 'int'), secure($row['event_admin'], 'int'), secure($action), secure($row['event_title']), secure($row['event_id']), secure($date) )) or _error(SQL_ERROR_THROWEN);
                            /* update notifications counter +1 */
                            $db->query(sprintf("UPDATE users SET user_live_notifications_counter = user_live_notifications_counter + 1 WHERE user_id = %s", secure($member_row['user_id'], 'int') )) or _error(SQL_ERROR_THROWEN);
                        }                                                                      
                    }
                }
            }
    ...
    static function pushNotificationAfterSessionStart()
    { 
        global $db, $date;

        $get_events = $db->query(sprintf('SELECT event_id, event_admin, event_title FROM `events` WHERE TIME_TO_SEC(TIMEDIFF(CURRENT_TIMESTAMP, event_start_date)) BETWEEN 1 AND 300 ' )) or _error(SQL_ERROR_THROWEN);

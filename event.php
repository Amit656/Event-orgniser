...
try {
	...
	$event['event_going'] = count($user->get_event_members($event['event_id'], 'going'));
	$event['event_interested'] = count($user->get_event_members($event['event_id'], 'interested'));
	$event['event_invited'] = count($user->get_event_members($event['event_id'], 'invited'));
	// [2] get view content
	/* check event privacy */
	...
	switch ($_GET['view']) {
		case '':
			...
			$smarty->assign('event_invited', 10);

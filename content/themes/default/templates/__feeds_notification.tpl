<li class="feeds-item {if !$notification['seen']}unread{/if}" data-id="{$notification['notification_id']}">
    <a class="data-container" {if !$notification['is_session_accepted'] && $notification['action'] == 'session_invitation'} data-toggle="modal" data-target="#Modal{$notification['notification_id']}" {else} href="{$notification['url']}"  {/if}>

...
<div class="post-header">
    ...
    <div class="post-meta">
        ...
        <span class="post-title">
            {if !$_shared && $_post['post_type'] == "shared"}
                ...
                <a href="{$system['system_url']}/posts/{$_post['origin']['post_id']}">
                    ...
                    {elseif $_post['origin']['post_type'] == 'video'}
                        {__("video")}
					{elseif $_post['audio']['source2'] && $_post['post_type'] == "audio" }
						{__("midi")}
                    {elseif substr($_post['audio']['source'],-3)=="mid" && $_post['post_type'] == "audio" }
						{__("midi")}
			...
			{elseif $_post['audio']['source2'] && $_post['post_type'] == "audio" }
				{__("added multiple Audio Files")}
			
			{elseif strpos( $_post['audio']['source'] , "|" ) !== false && $_post['post_type'] == "audio" && $_post['audio']}
				{$source=explode("|",$_post['audio']['source'])}
				{*
					{__("added")} {$source[0]} {$source[1]}
				*}
				{if isset($source[3]) }
					{__("added Audio/Midi +")}
				{elseif substr($source[0],-3)=="mid" && substr($source[1],-4)=="json" }
					{__("added Midi")}
				{else}
					{__("added Audio/Midi")}
				{/if}
...
{elseif strpos( $_post['audio']['source'] , "|" ) !== false && $_post['post_type'] == "audio" && $_post['audio']}
    {$source=explode("|",$_post['audio']['source'])}
	{$pid = $post['post_id'] }
	{if substr($source[0],-3)=="mid" && substr($source[1],-4)=="json" }
		<div style="width:100%;height:100%;" width="100%" height="100%"><a target="_blank" href="{$system['system_uploads']}/{$source[0]}">Midi File </a>
		<canvas id="{$pid}" width="1000px" height="100px" style="overflow:hidden;;width:100%;height:100px"></canvas></div>	
	{else}
		<div style="width:100%;height:100%;" width="100%" height="100%"><a target="_blank" href="{$system['system_uploads']}/{$source[1]}">Midi File </a> | <a target="_blank" href="{$system['system_uploads']}/{$source[0]}">Audio File</a>
		<canvas id="{$pid}" width="1000px" height="100px" style="overflow:hidden;;width:100%;height:100px"></canvas></div>	
		<div>
			<audio width="100%" class="js_mediaelementplayer">
				<source src="{$system['system_uploads']}/{$source[0]}">
				{__("Your browser does not support HTML5 audio")}
			</audio>
		</div>
	{/if}
	{if isset($source[2])}
	<script type="text/javascript">drawCanvas("{$pid}","{$system['system_uploads']}/{$source[2]}")</script>
	{elseif isset($source[1])}
	<script type="text/javascript">drawCanvas("{$pid}","{$system['system_uploads']}/{$source[1]}")</script>
	{/if}
{elseif substr($_post['audio']['source'],-3)=="mid" && $_post['post_type'] == "audio" && $_post['audio']}
	{$pid = $post['post_id']}
	{$source=explode("|",$_post['audio']['source'])}
    <div style="width:100%;height:100%;" width="100%" height="100%"><a target="_blank" href="{$system['system_uploads']}/{$_post['audio']['source']}">Midi File</a>
	{if isset($source[2])}
	<canvas id="{$pid}" width="1000px" height="100px" style="overflow:hidden;;width:100%;height:100px"></canvas></div>	
	<script type="text/javascript">drawCanvas("{$pid}","{$system['system_uploads']}/{$source[1]}")</script>
	{/if}
...
{elseif $_post['post_type'] == "audio" && $_post['audio']}
    <div> 
		<a target="_blank" href="{$system['system_uploads']}/{$_post['audio']['source']}">Audio File</a>
        <audio width="100%" class="js_mediaelementplayer">
